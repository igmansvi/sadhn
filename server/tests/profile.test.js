...
describe("Profile Routes", () => {
  ...
  describe("GET /api/profile/exists", () => {
    it("should return required true and exists false for learner without profile", async () => {
      const req = mockRequest({}, {}, { id: testUser._id });
      const res = mockResponse();

      await checkProfileExists(req, res);
    ...
    it("should return exists true for learner with profile", async () => {
      await Profile.create({
        user: testUser._id,
        profileType: "learner",
      });
    ...
    it("should return required false for non-learner", async () => {
      const employer = await User.create({
        name: "Employer",
        email: "exists-employer@example.com",
        password: "password123",
        role: "employer",
      });

      const req = mockRequest({}, {}, { id: employer._id.toString() });
      const res = mockResponse();
