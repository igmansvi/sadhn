...
import { ArrowLeft, Users, Mail, Phone, MapPin, Briefcase, GraduationCap, ExternalLink, FileText } from "lucide-react";
...
export default function ApplicationsPage() {
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            <div className="mb-6">
                <h1 className="text-2xl font-bold">{jobId ? "Applications" : "All Applications"}</h1>
                {job && <p className="text-muted-foreground">for {job.title}</p>}
                {!jobId && <p className="text-muted-foreground">Manage applications across all your jobs</p>}
            </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-1 space-y-3 max-h-[calc(100vh-280px)] overflow-y-auto">
                        {filteredApplications.map((app) => (
                            <Card
                                key={app._id}
                                className={`cursor-pointer transition-colors ${selectedApp?._id === app._id ? "ring-2 ring-primary" : "hover:bg-muted/50"}`}
                                onClick={() => setSelectedApp(app)}
                            >
                                <CardContent className="p-4">
                                    <div className="flex items-center gap-3">
                                        <Avatar>
                                            <AvatarFallback>{getInitials(app.applicant?.name)}</AvatarFallback>
                                        </Avatar>
                                ...
                                <CardHeader className="flex flex-row items-start justify-between">
                                    <div className="flex items-center gap-4">
                                        <Avatar className="h-16 w-16">
                                            <AvatarFallback className="text-xl">{getInitials(selectedApp.applicant?.name)}</AvatarFallback>
                                        </Avatar>
                                        <div>
                                    ...
                                    <Button onClick={() => openStatusDialog(selectedApp)}>Update Status</Button>
                                </CardHeader>
